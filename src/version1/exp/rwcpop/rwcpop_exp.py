...
def rwcpop_compute_fd_all():
    for i in range(1,101):
        if i < 10:
            ...
            number = str(i)
        test_path = prm.project_root + '/../data/rwcpop/Pop ' + number + ' (grid).csv'
        result_path = prm.project_root + '/../results/rwcpop/Pop ' + number + '/'
...
def rwcpop_compute_scores():
    ...
    if TO_WRITE:
        file_all_scores = open(prm.project_root + '/../results/rwcpop/scores_goodlevel.txt', "w")
        csvfile_all_scores = open(prm.project_root + '/../results/rwcpop/scores_goodlevel.csv', 'w', newline='')
    ...
    for i in range(min_val,max_val + 1):
        if i < 10:
            ...
            number = str(i)
        if i == 53 or i == 6 or i == 34 or i == 36 or i== 94:
            continue
        ...
        for level in range(len(est_labels_all)):
            ...
            # 'F-measure@0.5' or
            # 'Recall@0.5'
            if scores['Pairwise F-measure'] > best_scores:
                best_scores = scores['Pairwise F-measure']
