...
def rule1_dissimilarity(ms_oracle, level):
    ...
    if actual_ind > 0 and len(ms_oracle.levels[level].objects) > actual_ind:
        actual_obj = ms_oracle.levels[level].objects[actual_ind].descriptors.mean_descriptors[0][0]
        prec_obj = ms_oracle.levels[level].objects[actual_ind - 1].descriptors.mean_descriptors[0][0]
        sdd = 0
