...
def hypobit_add_level():
    prm.phases.append([])
    prm.bit_class.append([])
    prm.bit_seg.append([])

def bit_class_add(level, b):
    prm.bit_class[level].append(b)

def bit_seg_add(level, b):
    prm.bit_seg[level].append(b)


def compute_phases(level, end=0):
    i = len(prm.bit_class[level]) - 1
    if end:
        class_ind = i-1
        seg_ind = i
    else:
        class_ind = i - 1
        seg_ind = i
    if (1 - prm.bit_class[level][class_ind]) and (1 - prm.bit_seg[level][seg_ind]):
        prm.phases[level].append(1)
    elif (1 - prm.bit_class[level][class_ind]) and (prm.bit_seg[level][seg_ind]):
        prm.phases[level].append(2)
    elif (prm.bit_class[level][class_ind]) and (1 - prm.bit_seg[level][seg_ind]):
        prm.phases[level].append(3)
    elif (prm.bit_class[level][class_ind]) and (prm.bit_seg[level][seg_ind]):
        prm.phases[level].append(4)
    else:
        raise("this phase does not exist")
    tm.compute_time(level)

def print_phases():
    for level in range(len(prm.phases)):
        print("level", level, prm.phases[level])
        print("level", level, prm.time_tab[level])
